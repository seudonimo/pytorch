...
class AVKContext {
 ...
 private:
  ...
  void createDevice() {
    ...
    std::cout << "XXX queueFamilyIndex_:" << queueFamilyIndex_ << std::endl;
    ...
    std::cout << "uboAlign: " << uboAlign_ << " sboAlign: " << sboAlign_
              << std::endl;
  }
...
class AVKBuffer {
 public:
  ...
  void toHost(void* outputData, int64_t size) {
    std::cout << FLF << " size:" << size << std::endl;
...
class ComputeUnit {
 public:
  ...
  void createComputePipelineCompile(
    ...
    uint32_t codeSizeBytes = 4 * shaderSpvCode.size();
    createComputePipeline(shaderSpvCode.data(), codeSizeBytes);
  }
#endif
...
const char* getGlslSrcByName(const char* glslName) {
  return glslName;
}
...
#ifdef USE_VULKAN_SHADERC_RUNTIME
  auto glslSrc = getGlslSrcByName(glslName);
...
void VulkanVulkanTensor::setDataFromHost(const float* inputData) {
  ...
  auto computeUnit = makeComputeUnit(
      at::native::vulkan::vulkan_nchw_buf_to_tex_glsl, descrSetLayout);
  COUT_FLF;
...
void VulkanVulkanTensor::copyDataToHost(float* output) {
  ...
  std::cout << "bufferDataSize: " << bufferDataSize << " "
            << bufferDataSizeAligned << std::endl;
  AVKBuffer bufferData{bufferDataSizeAligned, physicalDevice, device, false};
  ...
  std::cout << "bufferConstSize: " << bufferConstSize << " "
            << bufferConstSizeAligned << std::endl;
  AVKBuffer bufferConst{bufferConstSizeAligned, physicalDevice, device, true};
  bufferConst.toDevice((void*)&wh, bufferConstSize);
  ...
  COUT_FLF;
  auto computeUnit = makeComputeUnit(
      at::native::vulkan::vulkan_tex_to_nchw_buf_glsl, descrSetLayout);
  COUT_FLF;
  computeUnit->createCommandBuffer(
      descriptorSet, UP_DIV(W, 8), UP_DIV(H, 8), C_4);
  COUT_FLF;
